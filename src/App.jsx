...
function ScoutingPage({ isVisible }) {
  ...
  const displayTeams = React.useMemo(() => {
    ...
    let list = [...teams];
    if (filter.trim()) { const q=filter.toLowerCase(); list=list.filter(t=>String(t.teamNumber).includes(q)||(t.name||'').toLowerCase().includes(q)); }
    if (col && !col.noSort) {
      list.sort((a,b) => { const va=col.getter(a),vb=col.getter(b); const cmp=col.isString?String(va).localeCompare(String(vb)):(Number(va)-Number(vb)); return sort.asc?cmp:-cmp; });
    }
    return list;
  }, [teams,filter,sort]);
  ...
  const maxVals = React.useMemo(() => {
    const m = {};
    SCOUT_COLUMNS.forEach(col => { if(col.hasBar) m[col.key]=Math.max(...teams.map(t=>Math.max(0,col.getter(t)||0)),1); });
    return m;
  }, [teams]);

  function toggleSort(key) {
    setSort(prev=>({key,asc:prev.key===key?!prev.asc:(SCOUT_COLUMNS.find(c=>c.key===key)?.defaultAsc??false)}));
  }
  ...
  function renderCell(col, team) {
    ...
    switch(col.key) {
      case 'team': return <a href={`https://ftcscout.org/teams/${team.teamNumber}`} target="_blank" rel="noopener noreferrer" onClick={e=>e.stopPropagation()} className="font-mono font-bold text-orange-500 hover:underline">{team.teamNumber}</a>;
      case 'name': return <span className="text-white text-sm font-medium">{team.name||<span className="text-[#A2A9B1]/40 italic text-xs">Unknown</span>}</span>;
      case 'seasonRank': return <ScoutRankBadge rank={v} />;
      case 'bestOpr': case 'autoOpr': case 'teleOpr': case 'egOpr': case 'dpr': case 'ccwm': case 'maxScore': case 'avgScore': {
        const n=Number(v); const cls=n>0?'text-green-400':n<0?'text-red-400':'text-[#A2A9B1]/40';
        return <span className={`font-mono text-sm ${cls}`}>{n.toFixed(1)}</span>;
      }
      case 'winRate': case 'lastWinRate': return <WinRateBadge pct={v} />;
      case 'record': return <span className="font-mono text-sm text-[#A2A9B1]">{team.totalWins}-{team.totalLosses}-{team.totalTies}</span>;
      case 'events': return <span className="font-mono text-sm text-[#A2A9B1]">{Math.round(Number(v))}</span>;
      case 'lastRank': return <LastRankBadge rank={v} />;
      case 'lastEvent': return v ? <a href={`https://ftcscout.org/events/${season}/${v}/rankings`} target="_blank" rel="noopener noreferrer" onClick={e=>e.stopPropagation()} className="text-orange-500/80 hover:text-orange-400 text-xs font-mono">{v}</a> : <span className="text-[#A2A9B1]/40 font-mono text-sm">—</span>;
      case 'comments': { const note=localStorage.getItem(`notes_${team.teamNumber}`); if(!note) return <span className="text-[#A2A9B1]/30 italic text-xs">No notes</span>; const d=note.trim().length>30?note.trim().substring(0,30)+'…':note.trim(); return <span className="text-[#A2A9B1]/70 italic text-xs">"{d}"</span>; }
      case 'awards': return team.awards?.length>0 ? <div className="flex flex-wrap gap-0.5">{team.awards.map((a,i)=><AwardBadge key={i} name={a.type||a.awardName||''} />)}</div> : <span className="text-[#A2A9B1]/30 text-sm">—</span>;
      ...
      <div className="max-w-7xl mx-auto px-4 relative z-10">
        <div className="text-center mb-14">
          <div id="scout-tag" data-animate className={`inline-block mb-6 transition-all duration-700 ${isVisible['scout-tag']?'animate-grow-in':'opacity-0 scale-[0.85]'}`}>
            <div className="flex items-center gap-3 px-6 py-3 bg-orange-600/20 border-2 border-orange-600" style={{clipPath:'polygon(0 0, calc(100% - 12px) 0, 100% 12px, 100% 100%, 0 100%)'}}>
              <Zap className="text-orange-500" size={16} /><span className="text-orange-500 font-black text-sm tracking-wider">FTC AUTO SCOUT</span>
            </div>
          </div>
          <h1 id="scout-title" data-animate className={`text-6xl md:text-8xl font-black text-white mb-4 tracking-tight transition-all duration-700 ${isVisible['scout-title']?'animate-lock-in':'opacity-0 translate-x-[-40px]'}`} style={{fontFamily:'system-ui,-apple-system,sans-serif',transitionDelay:'100ms'}}>SCOUTING</h1>
          <p id="scout-desc" data-animate className={`text-lg text-[#A2A9B1] max-w-2xl mx-auto transition-all duration-700 ${isVisible['scout-desc']?'animate-fade-in-up':'opacity-0 translate-y-[30px]'}`} style={{transitionDelay:'200ms'}}>
            Enter an FTC event code to load full team stats, OPR breakdowns, awards and more. Powered by <a href="https://ftcscout.org" target="_blank" rel="noopener noreferrer" className="text-orange-500 hover:underline">FTCScout</a>.
          </p>
        </div>
        <div className="relative mb-8">
          ...
          <div className="relative bg-gradient-to-br from-[#1a2847] to-[#0f1629] p-8 border-2 border-[#A2A9B1]" style={clipMd}>
            ...
            <p className="text-[#A2A9B1]/50 text-sm mb-6">Find event codes at <a href="https://ftcscout.org" target="_blank" rel="noopener noreferrer" className="text-orange-500/70 hover:text-orange-400">ftcscout.org</a></p>
            <form onSubmit={handleScout}>
              <div className="flex flex-col md:flex-row gap-3">
                <div className="flex flex-col gap-1.5 md:w-64">
                  ...
                  <select value={season} onChange={e=>setSeason(e.target.value)} disabled={loading} className={inputCls} style={{...clipSm,appearance:'none'}}>
                ...
                <div className="flex flex-col gap-1.5 flex-1">
                  ...
                  <input type="text" value={eventCode} onChange={e=>setEventCode(e.target.value.toUpperCase())} placeholder="e.g. USTXCCOS2" disabled={loading} className={inputCls} style={clipSm} autoComplete="off" spellCheck="false" />
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="text-[0.6rem] opacity-0 select-none">Go</label>
                  <button type="submit" disabled={loading} className="h-11 px-8 bg-gradient-to-br from-orange-600 to-orange-700 hover:from-orange-500 hover:to-orange-600 text-white font-black text-sm tracking-wider transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 whitespace-nowrap" style={clipSm}>
                    {loading?<><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />LOADING...</>:<><Search size={15} />SCOUT EVENT</>}
                  </button>
                </div>
              </div>
            </form>
            {loading&&<div className="absolute bottom-0 left-0 right-0 h-0.5 overflow-hidden bg-[#A2A9B1]/10"><div className="h-full w-1/3 bg-orange-500" style={{animation:'loadSlide 1.2s ease-in-out infinite'}} /></div>}
          </div>
        </div>
        {error&&<div className="mb-6 flex items-center gap-3 p-4 bg-red-500/10 border border-red-500/30 border-l-4 border-l-red-500 text-red-400 text-sm"><X size={15} className="shrink-0" />{error}</div>}
        {eventInfo&&(
          <div className="mb-8">
            <div className="bg-gradient-to-br from-[#1a2847] to-[#0f1629] p-6 border-2 border-[#A2A9B1] border-l-4 border-l-orange-600" style={{clipPath:'polygon(0 0, calc(100% - 16px) 0, 100% 16px, 100% 100%, 0 100%)'}}>
              <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                <div>
                  ...
                  <div className="flex flex-wrap gap-6">
                    {[['VENUE',[eventInfo.venue,eventInfo.city,eventInfo.state].filter(Boolean).join(', ')||'—'],['DATE',eventInfo.start===eventInfo.end?fmtDate(eventInfo.start):`${fmtDate(eventInfo.start)} — ${fmtDate(eventInfo.end)}`],['TEAMS',eventInfo.teamCount],['STATUS',eventInfo.status]].map(([label,val])=>(
                      <div key={label} className="flex flex-col gap-0.5"><span className="text-[0.55rem] font-bold tracking-widest text-[#A2A9B1]/50 uppercase">{label}</span><span className="text-[#A2A9B1] text-sm">{val}</span></div>
                    ))}
                  </div>
                </div>
                <div className="flex gap-2 shrink-0 flex-wrap">
                  <a href={`https://ftc-events.firstinspires.org/${eventInfo.season}/${eventInfo.code}`} target="_blank" rel="noopener noreferrer" className="px-3 py-2 border border-[#A2A9B1]/30 text-[#A2A9B1]/60 hover:border-orange-600 hover:text-orange-500 text-xs font-bold tracking-wider transition-colors">FTC EVENTS ↗</a>
                  <a href={`https://ftcscout.org/events/${eventInfo.season}/${eventInfo.code}/rankings`} target="_blank" rel="noopener noreferrer" className="px-3 py-2 border border-[#A2A9B1]/30 text-[#A2A9B1]/60 hover:border-orange-600 hover:text-orange-500 text-xs font-bold tracking-wider transition-colors">FTCSCOUT ↗</a>
                </div>
              </div>
            </div>
          </div>
        ...
        {teams.length>0&&(
          <div className="mb-12">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4 pb-3 border-b-2 border-[#A2A9B1]/20">
              <div className="flex items-center gap-3"><h3 className="text-sm font-black tracking-widest text-[#A2A9B1]">TEAM STATISTICS</h3><div className="h-px w-24 bg-gradient-to-r from-orange-600/40 to-transparent hidden md:block" /></div>
              <div className="flex flex-wrap items-center gap-4">
                <div className="flex items-center gap-2 cursor-pointer select-none" onClick={()=>setShowVisuals(v=>!v)}>
                  <div className={`w-9 h-5 rounded-full border transition-all relative ${showVisuals?'bg-orange-600/20 border-orange-600':'bg-[#A2A9B1]/10 border-[#A2A9B1]/20'}`}><div className={`absolute top-0.5 h-4 w-4 rounded-full transition-all ${showVisuals?'left-4 bg-orange-500':'left-0.5 bg-[#A2A9B1]/40'}`} /></div>
                ...
                <div className="relative">
                  ...
                  <input type="text" value={filter} onChange={e=>setFilter(e.target.value)} placeholder="Filter teams..." className="pl-8 pr-3 h-8 bg-[#0a1628] border border-[#A2A9B1]/20 text-white text-xs focus:outline-none focus:border-orange-600 transition-colors w-44 placeholder-[#A2A9B1]/30" style={clipSm} />
                </div>
                <span className="text-[0.65rem] font-mono text-[#A2A9B1]/40 whitespace-nowrap">{filter?`${displayTeams.length} / ${teams.length}`:`${teams.length}`} teams</span>
              </div>
            </div>
            <div className="border border-[#A2A9B1]/20 overflow-x-auto" style={{clipPath:'polygon(0 0, calc(100% - 16px) 0, 100% 16px, 100% 100%, 0 100%)',background:'rgba(10,22,40,0.7)'}}>
              <table className="w-full border-collapse" style={{minWidth:'1200px'}}>
                <thead>
                  <tr className="border-b-2 border-[#A2A9B1]/20">
                    {SCOUT_COLUMNS.map(col=>(
                      <th key={col.key} onClick={()=>!col.noSort&&toggleSort(col.key)}
                          className={`px-3 py-3 text-left text-[0.6rem] font-black tracking-widest uppercase whitespace-nowrap bg-[#1a2847]/60 transition-colors ${col.noSort?'cursor-default text-[#A2A9B1]/40':'cursor-pointer hover:text-orange-500'} ${sort.key===col.key?'text-orange-500':'text-[#A2A9B1]/60'}`}>
                        {col.label}{!col.noSort&&<span className="ml-1 opacity-60 text-[0.55rem]">{sort.key===col.key?(sort.asc?'▲':'▼'):'↕'}</span>}
                      </th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {displayTeams.map(team=>(
                    <tr key={team.teamNumber} onClick={()=>setSelectedTeam(team)} className="border-b border-[#A2A9B1]/10 cursor-pointer hover:bg-orange-600/5 transition-colors">
                      {SCOUT_COLUMNS.map(col=>{
                        const barPct=col.hasBar&&showVisuals?(Math.max(0,col.getter(team))/(maxVals[col.key]||1))*100:0;
                        return (
                          <td key={col.key} className="px-3 py-2.5 relative">
                            {col.hasBar&&showVisuals&&<div className={`absolute inset-y-1 left-0 transition-all duration-300 ${col.barColor==='yellow'?'bg-yellow-400/10':col.barColor==='red'?'bg-red-400/10':col.barColor==='green'?'bg-green-400/10':'bg-blue-400/10'}`} style={{width:`${barPct}%`}} />}
        ...
        <div className="text-center pt-8 border-t border-[#A2A9B1]/10">
          <p className="text-xs text-[#A2A9B1]/30">Data from <a href="https://ftcscout.org" target="_blank" rel="noopener noreferrer" className="hover:text-orange-500/60">FTCScout</a> · Original source FIRST · Click any row for full detail</p>
        </div>
      </div>
      {selectedTeam&&<TeamModal team={selectedTeam} season={season} allTeams={teams} onClose={()=>setSelectedTeam(null)} />}
    </div>
  );
}
// ─────────────────────────────────────────────────────────────────────────────
