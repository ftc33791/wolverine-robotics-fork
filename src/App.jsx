...
// ─────────────────────────────────────────────────────────────────────────────
// SCOUTING — constants, helpers, sub-components
// All defined at module level so React never recreates them
// ─────────────────────────────────────────────────────────────────────────────
...
function analyzePlaystyle({ autoOpr, teleOpr, egOpr, dpr, ccwm, seasonOpr, winRate, season }) {
  ...
  if (total > 0) {
    if (autoOpr >= 30) tags.push('Strong Auto');
    else if (autoOpr >= 15) tags.push('Solid Auto');
    else if (autoOpr < 5) tags.push('Weak Auto');
  }
  if (teleOpr >= 100) tags.push('TeleOp Powerhouse');
  else if (teleOpr >= 60) tags.push('Strong TeleOp');
  else if (teleOpr >= 30) tags.push('Avg TeleOp');
  if (egOpr >= 15) tags.push('Climber');
  else if (egOpr >= 5) tags.push('Parks');
  if (dpr > 0 && seasonOpr > 0 && dpr > seasonOpr * 0.8) tags.push('Defensive');
  if (ccwm >= 50) tags.push('Carry Potential');
  else if (ccwm >= 20) tags.push('Net Positive');
  else if (ccwm < 0) tags.push('Needs Strong Partner');
  if (winRate >= 80) tags.push('Elite');
  else if (winRate >= 65) tags.push('Consistent');
  if (season === '2024') {
  ...
  return tier + (tags.length > 0 ? tags.slice(0, 3).join(' · ') : 'Balanced');
}

function buildTeam(teamNumber, quickStats, awardsForTeam, season) {
  const name = quickStats?.teamName ?? '';
  ...
  const egOpr = quickStats?.eg?.value ?? 0;
  const dpr = 0; const ccwm = 0;
  const totalWins = 0; const totalLosses = 0; const totalTies = 0;
  const winRate = 0;
  ...
  return {
    teamNumber, name, location: '', bestOpr: seasonOpr,
    seasonOpr, seasonRank, autoOpr, teleOpr, egOpr, dpr, ccwm,
    totalWins, totalLosses, totalTies, winRate,
    lastOpr: 0, lastRank: 999, lastWins: 0, lastLosses: 0, lastTies: 0,
    lastEventCode: '', eventCount: 0, eventHistory: [],
    awards, awardCount: awards.length, playstyle
  };
}

const SCOUT_COLUMNS = [
  { key: 'team',       label: 'Team #',     getter: t => t.teamNumber,          sortFn: (a,b) => a-b },
  { key: 'name',       label: 'Name',       getter: t => t.name ?? '',          sortFn: (a,b) => a.localeCompare(b) },
  { key: 'seasonRank', label: 'Rank',       getter: t => t.seasonRank ?? 99999, sortFn: (a,b) => a-b, defaultAsc: true },
  { key: 'seasonOpr',  label: 'OPR',        getter: t => t.seasonOpr ?? 0,      sortFn: (a,b) => a-b, hasBar: true },
  { key: 'autoOpr',    label: 'Auto OPR',   getter: t => t.autoOpr ?? 0,        sortFn: (a,b) => a-b, hasBar: true, barColor: 'yellow' },
  { key: 'teleOpr',    label: 'TeleOp OPR', getter: t => t.teleOpr ?? 0,        sortFn: (a,b) => a-b, hasBar: true },
  { key: 'egOpr',      label: 'Endg OPR',   getter: t => t.egOpr ?? 0,          sortFn: (a,b) => a-b, hasBar: true, barColor: 'red' },
  { key: 'awards',     label: 'Awards',     getter: t => t.awardCount ?? 0,     sortFn: (a,b) => a-b },
];
...
function PhaseBar({ auto, tele, eg }) {
  ...
  return (
    <div className="flex h-1.5 w-full gap-px">
      <div className="bg-yellow-400/70 rounded-none" style={{ width: `${(auto / total) * 100}%` }} />
      <div className="bg-blue-400/70 rounded-none" style={{ width: `${(tele / total) * 100}%` }} />
      <div className="bg-red-400/70 rounded-none" style={{ width: `${(eg / total) * 100}%` }} />
    </div>
  );
}
...
function TeamModal({ team, allTeams, onClose }) {
  const [notes, setNotes] = useState(() => localStorage.getItem(`ftc_notes_${team.teamNumber}`) || '');
  ...
  function saveNotes() {
    localStorage.setItem(`ftc_notes_${team.teamNumber}`, notes);
    setSavedMsg('✓ Saved');
  ...
  return (
    ...
    >
      <div
        className="relative bg-gradient-to-br from-[#132038] to-[#0a1628] border-2 border-[#A2A9B1] w-full max-w-2xl max-h-[90vh] overflow-y-auto"
        style={clipLg}
      >
        {/* Orange corner fill */}
        ...
        <div className="p-8">
          {/* Close */}
          <button onClick={onClose} className="absolute top-3 right-3 text-[#A2A9B1]/60 hover:text-orange-500 transition-colors p-2 z-10">
            <X size={18} />
          </button>
          ...
          {/* OPR stats row */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-5 p-5 bg-[#1a2847]/50 border border-[#A2A9B1]/15" style={clipMd}>
            <StatBox label="OPR" value={team.seasonOpr.toFixed(1)} />
            <StatBox label="Auto OPR" value={team.autoOpr.toFixed(1)} />
            <StatBox label="TeleOp OPR" value={team.teleOpr.toFixed(1)} />
            <StatBox label="Endgame OPR" value={team.egOpr.toFixed(1)} />
          </div>

          {/* Global rank */}
          <div className="mb-5 p-4 bg-[#1a2847]/30 border border-[#A2A9B1]/15" style={clipSm}>
            <StatBox label="Global OPR Rank" value={team.seasonRank >= 99999 ? '—' : `#${team.seasonRank}`} />
          </div>
                    ...
                    <div key={label} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 flex gap-2 items-center">
                        <div className="flex-1 bg-[#1a2847] h-2 relative overflow-hidden">
                          <div className={`absolute top-0 left-0 h-full ${color} opacity-80 transition-all`} style={{ width: `${(tv / maxVal) * 100}%` }} />
                        </div>
                        <span className="font-mono text-xs text-white w-10 shrink-0">{tv.toFixed(1)}</span>
                      </div>
                      <div className="flex-1 flex gap-2 items-center">
                        <div className="flex-1 bg-[#1a2847] h-2 relative overflow-hidden">
                          <div className="absolute top-0 left-0 h-full bg-[#A2A9B1] opacity-40 transition-all" style={{ width: `${(avg / maxVal) * 100}%` }} />
                        </div>
...
function AwardsPanel({ teams }) {
  ...
  for (const t of teams) {
    for (const a of (t.awards || [])) {
      allAwards.push({ ...a, teamNumber: t.teamNumber, teamName: t.name });
    }
  }
...
// ── MAIN SCOUTING PAGE — fully outside App, stable identity ──
...
function ScoutingPage({ isVisible }) {
  ...
  async function handleScout(e) {
    ...
    try {
      ...
      const assembled = teamNumbers.map(num =>
        buildTeam(num, quickResults[num], awardsMap[num] || [], season)
      );
  ...
  function renderCell(col, team) {
    ...
    switch (col.key) {
      ...
      case 'name':
        return <span className="text-white text-sm font-medium">{team.name || <span className="text-[#A2A9B1]/40 italic">Unknown</span>}</span>;
      case 'seasonRank':
        return <ScoutRankBadge rank={v} />;
      case 'seasonOpr': case 'autoOpr': case 'teleOpr': case 'egOpr': {
        const n = Number(v);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#132038] to-[#0a1628] py-32 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 relative z-10">
        ...
        <div className="text-center mb-14">
          ...
          <p id="scout-desc" data-animate
            ...
            Enter an FTC event code to load OPR, awards and team data.
                  ...
                  <tr className="border-b-2 border-[#A2A9B1]/20">
                    {SCOUT_COLUMNS.map(col => (
                      <th
                        ...
                        className={`px-3 py-3 text-left text-[0.6rem] font-black tracking-widest uppercase whitespace-nowrap cursor-pointer transition-colors bg-[#1a2847]/60 hover:text-orange-500 ${sort.key === col.key ? 'text-orange-500' : 'text-[#A2A9B1]/60'}`}
                      >
                        {col.label}
                    ...
                    >
                      {SCOUT_COLUMNS.map(col => {
                        const barPct = col.hasBar && showVisuals
                          ? (Math.max(0, col.getter(team)) / (maxVals[col.key] || 1)) * 100
                          : 0;
                        return (
                          <td key={col.key} className="px-3 py-2.5 relative">
                            {col.hasBar && showVisuals && (
                              <div
                                className={`absolute inset-y-1 left-0 transition-all duration-300 ${
                                  col.barColor === 'yellow' ? 'bg-yellow-400/10' :
                                  col.barColor === 'red'    ? 'bg-red-400/10'    : 'bg-orange-500/10'
                                }`}
                                style={{ width: `${barPct}%` }}
                              />
                            )}
        ...
        {!loading && !error && teams.length === 0 && (
          <div className="text-center py-24 text-[#A2A9B1]/40">
            ...
            <p className="text-sm mt-1">
              Find codes at <a href="https://ftcscout.org" target="_blank" rel="noopener noreferrer" className="text-orange-500/70 hover:text-orange-400">ftcscout.org</a>
            </p>
          </div>
        )}
        ...
        <div className="text-center pt-8 border-t border-[#A2A9B1]/10">
          <p className="text-xs text-[#A2A9B1]/30">
            Data from <a href="https://ftcscout.org" target="_blank" rel="noopener noreferrer" className="hover:text-orange-500/60">FTCScout API</a> · Original source FIRST · Click any row to view team details
          </p>
        </div>
      </div>
      ...
      {/* Team modal */}
      {selectedTeam && (
        <TeamModal team={selectedTeam} allTeams={teams} onClose={() => setSelectedTeam(null)} />
      )}
    </div>
  );
}
...
// ─────────────────────────────────────────────────────────────────────────────
// EXISTING SITE COMPONENTS (unchanged from your original)
// ─────────────────────────────────────────────────────────────────────────────
                ...
                <div className="space-y-10">
                  ...
                  <div className="bg-gradient-to-br from-blue-900/30 to-blue-950/30 p-8 border-2 border-[#A2A9B1] hover:border-blue-500 transition-all duration-300 group relative hover:scale-[1.02]"
                    ...
                    <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300"
                         style={{boxShadow:'inset 0 0 30px rgba(66,153,225,0.4)'}} />
                          ...
                          <div className="flex gap-4">
                            {[[Github,'https://github.com/wolverine-robotics'],[Linkedin,'https://www.linkedin.com/company/wolverine-robotics/'],[Instagram,'https://www.instagram.com/wolverine_robotics/']].map(([Icon,href],i) => (
                              <a key={i} href={href} target="_blank" rel="noopener noreferrer"
                                 className="w-12 h-12 bg-orange-600 hover:bg-orange-500 flex items-center justify-center transition-all duration-300 hover:scale-125"
