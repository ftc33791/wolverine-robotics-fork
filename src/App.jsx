...
import { Menu, X, ChevronRight, Award, Calendar, Users, Mail, MapPin, Github, Linkedin, Instagram, Zap, Search, BarChart2 } from 'lucide-react';
import * as THREE from 'three';

// ============================================================
// SCOUTING PAGE — helper functions & sub-components
// ============================================================
...
function fmtDec1(n) { return Number(n).toFixed(1); }
...
function buildTeam(teamNumber, igniteData, quickStats, awardsForTeam, season) {
  ...
  let lastOpr = 0, lastRank = 999, lastWins = 0, lastLosses = 0, lastTies = 0, lastEventCode = '';
  let lastWinRate = 0, bestOpr = 0;
  let eventHistory = [];
  if (ig?.events && ig?.eventStats) {
    const validEvents = [...ig.events].filter(e => ig.eventStats[e.eventId]?.wins !== null);
  ...
  return {
    ...
    awards, awardCount: awards.length, playstyle, eventHistory
  };
}

function getScoutColumns() {
  return [
    { key: 'team',       label: 'Team #',     getter: t => t.teamNumber,          tooltip: 'Team number' },
    { key: 'name',       label: 'Name',       getter: t => t.name,                tooltip: 'Team name' },
    { key: 'seasonRank', label: 'Rank',       getter: t => t.seasonRank ?? 99999, tooltip: 'Global OPR ranking' },
    { key: 'bestOpr',    label: 'Best OPR',   getter: t => t.bestOpr ?? 0,        tooltip: 'Highest OPR from any event', hasBar: true },
    { key: 'seasonOpr',  label: 'Season OPR', getter: t => t.seasonOpr ?? 0,      tooltip: 'Season-wide OPR', hasBar: true },
    { key: 'autoOpr',    label: 'Auto',       getter: t => t.autoOpr ?? 0,        tooltip: 'Auto OPR', hasBar: true, barColor: 'yellow' },
    { key: 'teleOpr',    label: 'TeleOp',     getter: t => t.teleOpr ?? 0,        tooltip: 'TeleOp OPR', hasBar: true },
    { key: 'egOpr',      label: 'Endgame',    getter: t => t.egOpr ?? 0,          tooltip: 'Endgame OPR', hasBar: true, barColor: 'red' },
    { key: 'ccwm',       label: 'CCWM',       getter: t => t.ccwm ?? 0,           tooltip: 'Contribution to Winning Margin', hasBar: true, barColor: 'green' },
    { key: 'winRate',    label: 'Win%',       getter: t => t.winRate ?? 0,        tooltip: 'Season win rate' },
    { key: 'record',     label: 'W-L-T',      getter: t => t.totalWins ?? 0,      tooltip: 'Season record', noSort: true },
    { key: 'lastRank',   label: 'Last Rank',  getter: t => t.lastRank ?? 999,     tooltip: 'Rank at last event' },
    { key: 'lastEvent',  label: 'Last Event', getter: t => t.lastEventCode ?? '', tooltip: 'Most recent event' },
    { key: 'awards',     label: 'Awards',     getter: t => t.awardCount ?? 0,     tooltip: 'Season awards count' },
...
function ScoutRankTag({ rank }) {
  const r = Number(rank);
  if (r >= 999) return <span className="text-[#A2A9B1]/50 font-mono">—</span>;
...
function Sparkline({ data, color = '#FF5A1F', width = 120, height = 40 }) {
  if (!data || data.length < 2) return <span className="text-[#A2A9B1]/40 text-xs">—</span>;
...
function PhaseBar({ auto, tele, eg }) {
  ...
  return (
    <div className="flex h-2 w-full gap-0.5">
      <div className="bg-yellow-400/70" style={{ width: `${(auto / total) * 100}%` }} title={`Auto: ${auto.toFixed(1)}`} />
      <div className="bg-blue-400/70" style={{ width: `${(tele / total) * 100}%` }} title={`TeleOp: ${tele.toFixed(1)}`} />
      <div className="bg-red-400/70" style={{ width: `${(eg / total) * 100}%` }} title={`Endgame: ${eg.toFixed(1)}`} />
    </div>
  );
}
...
function TeamModal({ team, season, allTeams, onClose }) {
  ...
  const statBox = (label, value, sub = false) => (
    <div className="flex flex-col gap-1">
      <span className="text-[0.6rem] font-bold tracking-widest text-[#A2A9B1]/60 uppercase">{label}</span>
      <span className={`font-mono font-bold ${sub ? 'text-xl text-[#A2A9B1]' : 'text-3xl text-white'}`}>{value}</span>
    </div>
  );
  ...
  return (
    ...
    >
      <div
        className="relative bg-gradient-to-br from-[#132038] to-[#0a1628] border-2 border-[#A2A9B1] w-full max-w-3xl max-h-[90vh] overflow-y-auto p-8 md:p-10"
        style={{ clipPath: 'polygon(0 0, calc(100% - 28px) 0, 100% 28px, 100% 100%, 0 100%)' }}
      >
        {/* Orange corner accent */}
        <div className="absolute top-0 right-0 w-7 h-7"
             style={{ background: 'linear-gradient(135deg, transparent 50%, #EA580C 50%)' }} />

        <button onClick={onClose} className="absolute top-4 right-4 text-[#A2A9B1] hover:text-orange-500 transition-colors p-2 z-10">
          <X size={20} />
        </button>

        {/* Header */}
        <div className="mb-8 pb-6 border-b-2 border-[#A2A9B1]/20">
          <div className="flex items-baseline gap-4 mb-1">
            <span className="text-orange-500 font-black font-mono text-3xl">#{team.teamNumber}</span>
            <h2 className="text-white font-black text-2xl" style={{ fontFamily: 'system-ui, -apple-system, sans-serif' }}>
              {team.name || 'Unknown Team'}
            </h2>
          </div>
          <p className="text-[#A2A9B1]/60 text-sm">{team.location || 'Location Unknown'}</p>
        </div>

        {/* Primary stats */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8 p-6 bg-[#1a2847]/50 border border-[#A2A9B1]/20" style={clipMd}>
          {statBox('Season OPR', fmtDec1(team.seasonOpr))}
          {statBox('Global Rank', team.seasonRank >= 99999 ? '—' : `#${team.seasonRank}`)}
          {statBox('Season Win%', `${team.winRate.toFixed(0)}%`)}
          {statBox('Events', team.eventCount)}
        </div>

        {/* Sub stats */}
        <div className="grid grid-cols-3 gap-4 mb-6">
          {[
            { label: 'Auto OPR',    val: team.autoOpr, border: 'border-yellow-400/20' },
            { label: 'TeleOp OPR',  val: team.teleOpr, border: 'border-blue-400/20' },
            { label: 'Endgame OPR', val: team.egOpr,   border: 'border-red-400/20' },
          ].map(({ label, val, border }) => (
            <div key={label} className={`p-4 bg-[#1a2847]/40 border ${border}`} style={clipSm}>
              {statBox(label, fmtDec1(val), true)}
            </div>
          ))}
        </div>
        ...
        {/* Last event */}
        {team.lastEventCode && (
          <div className="mb-8 p-6 bg-[#1a2847]/30 border border-[#A2A9B1]/20" style={clipMd}>
            <p className="text-[0.6rem] font-bold tracking-widest text-[#A2A9B1]/60 uppercase mb-4">
              Latest Event: <span className="text-orange-500">{team.lastEventCode}</span>
            </p>
            <div className="grid grid-cols-3 gap-4">
              {statBox('Event OPR', team.lastOpr > 0 ? fmtDec1(team.lastOpr) : '—', true)}
              {statBox('Rank', team.lastRank >= 999 ? '—' : `#${team.lastRank}`, true)}
              {statBox('Record', `${team.lastWins}-${team.lastLosses}-${team.lastTies}`, true)}
            </div>
          </div>
        )}
        ...
        {oprData.length >= 2 && (
          <div className="mb-8">
            ...
            <div className="flex items-end gap-6 flex-wrap">
              <Sparkline data={oprData} color="#FF5A1F" width={200} height={50} />
              ...
              return (
                <div key={label} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 flex gap-1 items-center">
                    ...
                    <span className="font-mono text-xs text-white w-12 shrink-0">{tv.toFixed(1)}</span>
                  </div>
                  <div className="flex-1 flex gap-1 items-center">
                    <div className="flex-1 bg-[#1a2847] h-2 relative overflow-hidden">
                      <div className="absolute top-0 left-0 h-full bg-[#A2A9B1] opacity-40" style={{ width: `${(avg / maxVal) * 100}%` }} />
                    </div>
                    <span className="font-mono text-xs text-[#A2A9B1]/60 w-12 shrink-0">{avg.toFixed(1)}</span>
                  </div>
                </div>
              );
            })}
        ...
        {/* Robot Profile */}
        <div className="mb-8 p-4 bg-orange-600/5 border border-orange-600/20" style={clipSm}>
          <p className="text-[0.6rem] font-bold tracking-widest text-[#A2A9B1]/60 uppercase mb-2">Robot Profile</p>
        ...
        {team.awards?.length > 0 && (
          <div className="mb-8">
            <p className="text-[0.6rem] font-bold tracking-widest text-[#A2A9B1]/60 uppercase mb-3">Awards</p>
        ...
        {/* Scouting Notes */}
        <div className="mb-8 p-6 bg-blue-900/10 border border-blue-500/15" style={clipMd}>
          <p className="text-[0.6rem] font-bold tracking-widest text-[#A2A9B1]/60 uppercase mb-3">Scouting Notes</p>
          <textarea
            value={notes}
            onChange={e => setNotes(e.target.value)}
            placeholder="Enter scouting observations, strengths, weaknesses..."
            rows={5}
            className="w-full bg-[#0a1628] border border-[#A2A9B1]/20 text-white text-sm p-4 focus:outline-none focus:border-orange-600 resize-none transition-colors"
            style={clipSm}
          />
          <div className="flex items-center justify-between mt-3">
            <span className="text-xs text-[#A2A9B1]/50 italic">{savedMsg || 'Saved locally in browser'}</span>
            <button
              onClick={saveNotes}
              className="px-6 py-2 bg-orange-600 hover:bg-orange-500 text-white text-xs font-black tracking-widest transition-colors"
              style={clipSm}
            >
              {savedMsg ? '✓ SAVED' : 'SAVE NOTES'}
            </button>
          </div>
        </div>

        {/* Footer links */}
        <div className="flex gap-6 pt-4 border-t border-[#A2A9B1]/20">
          ...
          <a href={`https://theorangealliance.org/teams/${team.teamNumber}`} target="_blank" rel="noopener noreferrer"
             className="text-xs text-[#A2A9B1]/50 hover:text-orange-500 transition-colors">The Orange Alliance ↗</a>
        </div>
      </div>
    </div>
  );
    ...
    <div className="mb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
        {unique.map((award, i) => {
          ...
          const borderCls =
            lower.includes('inspire') ? 'border-l-blue-400/60' :
            lower.includes('finalist') ? 'border-l-[#A2A9B1]/40' :
            lower.includes('control') ? 'border-l-orange-500/60' :
            lower.includes('connect') ? 'border-l-green-400/60' :
            lower.includes('innovate') ? 'border-l-purple-400/60' :
            'border-l-yellow-500/60';
          const icon =
            lower.includes('winning') || lower.includes('winner') ? '🏆' :
          ...
          return (
            <div key={i} className={`flex items-center gap-3 p-3 bg-[#1a2847]/40 border border-[#A2A9B1]/15 border-l-4 ${borderCls}`}>
...
// ============================================================
// EXISTING SITE COMPONENTS (unchanged)
// ============================================================
...
void main(){vec4 c;mainImage(c,vUv*iResolution.xy);gl_FragColor=c;}`;
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      last = now;
      const smoothDampVec2 = (current, target, vel, st, dt) => {
        const out = current.clone(); st = Math.max(0.0001, st);
        const omega = 2 / st; const x = omega * dt;
      ...
      lookCurrent.current.copy(smoothDampVec2(lookCurrent.current, lookTarget.current, lookVel.current, smoothTime, dt));
...
const InitialLoadAnimation = ({ onComplete }) => {
  ...
  useEffect(() => {
    const criticalImages = [
      '/data/logo.svg', '/claw.png',
      '/data/robots/matchstick-main.jpg', '/data/robots/matchstick-1.jpg',
      '/data/robots/matchstick-2.jpg', '/data/robots/matchstick-3.jpg',
      '/data/robots/matchstick-4.jpg', '/data/team/dev.jpg',
      '/data/team/sahejdeep.jpg', '/data/team/sripadh.jpg',
      '/data/team/manveer.jpg', '/data/sponsors/whs.png', '/data/sponsors/nhs.png'
    ];
    let loadedCount = 0;
    const checkAllLoaded = () => { if (++loadedCount >= criticalImages.length) setImagesPreloaded(true); };
    criticalImages.forEach(src => { const img = new Image(); img.onload = checkAllLoaded; img.onerror = checkAllLoaded; img.src = src; });
  }, []);
        ...
        <div className="relative flex flex-col items-center">
          <div className="w-80 h-80 relative mb-8 flex items-center justify-center mx-auto">
            ...
            {clawImageLoaded && !clawImageError ? (
              <img src="/data/logo.svg" alt="Wolverine Claw"
...
const App = () => {
  ...
  useEffect(() => {
    const setFavicon = () => {
      ...
      favicon.rel = 'icon'; favicon.type = 'image/svg+xml'; favicon.href = '/data/logo.svg';
  ...
  useEffect(() => {
    const handlePopState = () => {
      ...
      if (['home', 'about', 'robots', 'sponsors', 'contact', 'scouting'].includes(page)) {
        setCurrentPage(page);
      }
    };
  ...
  useEffect(() => {
    ...
    style.textContent = `
      ...
      @keyframes slideInDiagonal { from { opacity:0; transform:translate(-60px,60px); } to { opacity:1; transform:translate(0,0); } }
      ...
      .animate-slide-diagonal { animation:slideInDiagonal 0.8s cubic-bezier(0.16,1,0.3,1) forwards; opacity:0; }
      .animate-lock-in { animation:lockInPlace 0.6s cubic-bezier(0.34,1.56,0.64,1) forwards; opacity:0; }
      .animate-pulse-glow { animation:pulseglow 2s ease-in-out infinite; }
      .animate-slide-down-fade { animation:slideDownFade 0.5s cubic-bezier(0.16,1,0.3,1) forwards; }
  ...
  useEffect(() => {
    const setupTimer = setTimeout(() => {
      const observer = new IntersectionObserver(
        entries => { entries.forEach(entry => { if (entry.isIntersecting && entry.target.id) setIsVisible(prev => ({ ...prev, [entry.target.id]: true })); }); },
  ...
  // ── Scouting Page (inline) ──────────────────────────────────
  const ScoutingPageComponent = () => {
    const SEASONS = [
    ...
    const [sort, setSort] = useState({ key: 'seasonOpr', dir: 'desc' });
    const [tableFilter, setTableFilter] = useState('');
    const [showVisuals, setShowVisuals] = useState(false);
    const [selectedTeam, setSelectedTeam] = useState(null);
    const columns = getScoutColumns();
    ...
    useEffect(() => {
      ...
      if (q) sorted = sorted.filter(t => String(t.teamNumber).includes(q) || (t.name || '').toLowerCase().includes(q));
      setFilteredTeams(sorted);
    }, [teams, sort, tableFilter]);

    function toggleSort(key) {
      setSort(prev => ({
        key,
        dir: prev.key === key ? (prev.dir === 'asc' ? 'desc' : 'asc') : (key === 'seasonRank' || key === 'lastRank' ? 'asc' : 'desc')
      }));
    }
    ...
    async function handleScout(e) {
      ...
      setError(''); setLoading(true); setEventInfo(null); setTeams([]);
      try {
        ...
        const quickResults = await batchFetch(
          teamNumbers,
          n => fetchJSON(`${FTCSCOUT_API}/teams/${n}/quick-stats?season=${season}`).catch(() => null),
          5
        );
        const assembled = teamNumbers.map(num => buildTeam(num, null, quickResults[num], awardsMap[num] || [], season));
    ...
    const maxVals = {};
    columns.forEach(col => {
      if (col.hasBar) maxVals[col.key] = Math.max(...filteredTeams.map(t => col.getter(t) || 0), 1);
    });
    ...
    function renderCell(col, team) {
      ...
      switch (col.key) {
        case 'team':
          return (
            <span className="font-mono font-bold text-orange-500">
              <a href={`https://ftcscout.org/teams/${team.teamNumber}`} target="_blank" rel="noopener noreferrer"
                 onClick={e => e.stopPropagation()} className="hover:underline">{team.teamNumber}</a>
            </span>
          );
        case 'name': return <span className="text-white font-medium text-sm">{team.name || 'Unknown'}</span>;
        case 'seasonRank': return v >= 99999 ? <span className="text-[#A2A9B1]/40 font-mono">—</span> : <span className="text-[#A2A9B1] font-mono">#{v}</span>;
        case 'bestOpr': case 'seasonOpr': case 'autoOpr': case 'teleOpr': case 'egOpr': case 'ccwm': {
          const n = Number(v);
          const cls = n > 0 ? 'text-green-400' : n < 0 ? 'text-red-400' : 'text-[#A2A9B1]/60';
          return <span className={`font-mono text-sm ${cls}`}>{n.toFixed(1)}</span>;
        }
        case 'winRate': return <WinRateBadge pct={v} />;
        case 'record': return <span className="font-mono text-sm text-[#A2A9B1]">{team.totalWins}-{team.totalLosses}-{team.totalTies}</span>;
        case 'lastRank': return <ScoutRankTag rank={v} />;
        case 'lastEvent':
          return v
            ? <a href={`https://ftcscout.org/events/${season}/${v}/rankings`} target="_blank" rel="noopener noreferrer"
                 onClick={e => e.stopPropagation()} className="text-orange-500/80 hover:text-orange-400 text-xs font-mono">{v}</a>
        ...
        case 'location': return <span className="text-[#A2A9B1]/50 text-xs">{v || '—'}</span>;
        default: return <span className="text-[#A2A9B1] text-sm">{String(v)}</span>;
      }
    }

    const clipSm = { clipPath: 'polygon(0 0, calc(100% - 8px) 0, 100% 8px, 100% 100%, 0 100%)' };
    const inputCls = "bg-[#0a1628] border border-[#A2A9B1]/30 text-white text-sm px-4 h-11 focus:outline-none focus:border-orange-600 transition-colors font-mono w-full";

    return (
      <div className="min-h-screen bg-gradient-to-b from-[#132038] to-[#0a1628] py-32 relative overflow-hidden">
        <ClawMarkImage opacity={0.08} className="bottom-0 right-0 w-[700px] h-[700px]" />
        ...
        <div className="max-w-7xl mx-auto px-4 relative z-10">
          ...
          <div className="text-center mb-16">
            <div
              id="scout-tag" data-animate
              className={`inline-block mb-6 transition-all duration-700 ${isVisible['scout-tag'] ? 'animate-grow-in' : 'opacity-0 scale-[0.85]'}`}
            >
            ...
            <p
              id="scout-desc" data-animate
              className={`text-xl text-[#A2A9B1] max-w-2xl mx-auto transition-all duration-700 ${isVisible['scout-desc'] ? 'animate-fade-in-up' : 'opacity-0 translate-y-[30px]'}`}
              style={{ transitionDelay: '200ms' }}
            >
              Enter an FTC event code to pull OPR, win rates, awards, and team data.
              Powered by <a href="https://ftcscout.org" target="_blank" rel="noopener noreferrer" className="text-orange-500 hover:underline">FTCScout API</a>.
            </p>
          </div>

          {/* Search card */}
          <div
            id="scout-search" data-animate
            className={`relative mb-10 transition-all duration-700 ${isVisible['scout-search'] ? 'animate-grow-in' : 'opacity-0 scale-[0.85]'}`}
            style={{ transitionDelay: '300ms' }}
          >
            <div className="absolute inset-0 bg-gradient-to-br from-orange-600/10 to-blue-900/10 translate-x-3 translate-y-3"
                 style={{ clipPath: 'polygon(0 0, calc(100% - 16px) 0, 100% 16px, 100% 100%, 0 100%)' }} />
            <div className="relative bg-gradient-to-br from-[#1a2847] to-[#0f1629] p-8 border-2 border-[#A2A9B1]"
                 style={{ clipPath: 'polygon(0 0, calc(100% - 16px) 0, 100% 16px, 100% 100%, 0 100%)' }}>
              <h2 className="text-xs font-black tracking-widest text-orange-500 mb-1">LOOK UP EVENT</h2>
              <p className="text-[#A2A9B1]/60 text-sm mb-6">
                Enter an FTC event code — find codes at{' '}
                <a href="https://ftcscout.org" target="_blank" rel="noopener noreferrer" className="text-orange-500/80 hover:text-orange-400">ftcscout.org</a>
              </p>
                ...
                <div className="flex flex-col md:flex-row gap-3">
                  <div className="flex flex-col gap-1.5 md:w-72">
                    ...
                    <select value={season} onChange={e => setSeason(e.target.value)} disabled={loading}
                      ...
                      {SEASONS.map(s => <option key={s.value} value={s.value} style={{ background: '#0a1628' }}>{s.label}</option>)}
                    </select>
                  </div>
                  <div className="flex flex-col gap-1.5 flex-1">
                    <label className="text-[0.6rem] font-bold tracking-widest text-[#A2A9B1]/60 uppercase">Event Code</label>
                    <input
                      type="text" value={eventCode} disabled={loading} placeholder="e.g. USTXCCOS2"
                      onChange={e => {
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[0.6rem] font-bold tracking-widest text-[#A2A9B1]/60 uppercase opacity-0 select-none">Go</label>
                    <button type="submit" disabled={loading}
                            className="h-11 px-8 bg-gradient-to-br from-orange-600 to-orange-700 hover:from-orange-500 hover:to-orange-600 text-white font-black text-sm tracking-wider transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 whitespace-nowrap"
                            style={clipSm}>
                      {loading ? (
                        <><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />LOADING...</>
                      ) : (
                        <><Search size={16} />SCOUT EVENT</>
                      )}
                    </button>
                  </div>
                </div>
              ...
              {loading && (
                <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#A2A9B1]/10 overflow-hidden">
                  <div className="h-full w-1/3 bg-orange-600" style={{ animation: 'loadSlide 1.2s ease-in-out infinite' }} />
                </div>
              )}
            </div>
          </div>
                ...
                <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                  <div>
                    ...
                    <div className="flex flex-wrap gap-6">
                      {[
                        ...
                        ['DATE', eventInfo.start === eventInfo.end ? formatDate(eventInfo.start) : `${formatDate(eventInfo.start)} — ${formatDate(eventInfo.end)}`],
                  ...
                  <div className="flex gap-2 shrink-0 flex-wrap">
                    <a href={`https://ftc-events.firstinspires.org/${eventInfo.season}/${eventInfo.code}`} target="_blank" rel="noopener noreferrer"
                       className="px-4 py-2 border border-[#A2A9B1]/30 text-[#A2A9B1]/60 hover:border-orange-600 hover:text-orange-500 text-xs font-bold tracking-wider transition-colors">
                      FTC EVENTS ↗
                    </a>
          ...
          {teams.length > 0 && (
            <div className="mb-12">
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4 pb-4 border-b-2 border-[#A2A9B1]/20">
                <div className="flex items-center gap-4">
                  <h3 className="text-sm font-black tracking-widest text-[#A2A9B1]">TEAM STATISTICS</h3>
                  <div className="h-px bg-gradient-to-r from-orange-600/40 to-transparent hidden md:block w-32" />
                </div>
                <div className="flex flex-wrap items-center gap-4">
                  {/* Visuals toggle */}
                  <div className="flex items-center gap-2 cursor-pointer select-none" onClick={() => setShowVisuals(v => !v)}>
                    <div className={`w-9 h-5 rounded-full border transition-colors relative ${showVisuals ? 'bg-orange-600/20 border-orange-600' : 'bg-[#A2A9B1]/10 border-[#A2A9B1]/20'}`}>
                  ...
                  {/* Filter */}
                  <div className="relative">
                    <Search size={12} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#A2A9B1]/40" />
                    <input type="text" value={tableFilter} onChange={e => setTableFilter(e.target.value)}
                           placeholder="Filter teams..."
                           className="pl-8 pr-3 h-8 bg-[#0a1628] border border-[#A2A9B1]/20 text-white text-xs focus:outline-none focus:border-orange-600 transition-colors w-44"
                           style={clipSm} />
                  </div>
                  <span className="text-[0.65rem] font-mono text-[#A2A9B1]/40 whitespace-nowrap">
                    {tableFilter ? `${filteredTeams.length} / ${teams.length}` : `${teams.length}`} teams
                  </span>
                </div>
              </div>

              <div className="border border-[#A2A9B1]/20 overflow-x-auto bg-[#0a1628]/60"
                   style={{ clipPath: 'polygon(0 0, calc(100% - 16px) 0, 100% 16px, 100% 100%, 0 100%)' }}>
                <table className="w-full border-collapse" style={{ minWidth: '1100px' }}>
                  <thead>
                    <tr className="border-b-2 border-[#A2A9B1]/20">
                      {columns.map(col => (
                        <th key={col.key} onClick={() => !col.noSort && toggleSort(col.key)} title={col.tooltip}
                          ...
                          {col.label}
                          {!col.noSort && (
                            <span className="ml-1 opacity-50 text-[0.55rem]">
                          ...
                          return (
                            <td key={col.key} className="px-3 py-2.5 relative">
                              {col.hasBar && showVisuals && (
                                <div className={`absolute inset-y-1 left-0 transition-all duration-500 ${
                                  col.barColor === 'yellow' ? 'bg-yellow-400/10' :
                                  col.barColor === 'red' ? 'bg-red-400/10' :
                                  col.barColor === 'green' ? 'bg-green-400/10' : 'bg-orange-500/10'
                                }`} style={{ width: `${barPct}%` }} />
                              )}
          ...
          {!loading && !error && teams.length === 0 && (
            <div className="text-center py-24 text-[#A2A9B1]/40">
              ...
              <p className="text-sm mt-2">
                Find event codes at{' '}
                <a href="https://ftcscout.org" target="_blank" rel="noopener noreferrer" className="text-orange-500/70 hover:text-orange-400">ftcscout.org</a>
              </p>
            </div>
          )}
          ...
          <div className="text-center pt-8 border-t border-[#A2A9B1]/10">
            <p className="text-xs text-[#A2A9B1]/30">
              Data from <a href="https://ftcscout.org" target="_blank" rel="noopener noreferrer" className="hover:text-orange-500/60 transition-colors">FTCScout API</a> · Original source FIRST · Click any row to view team details
            </p>
          </div>
        </div>
  ...
  const renderPage = () => {
        ...
        <div className="min-h-screen">
          <div className="relative min-h-[100dvh] flex items-center justify-center overflow-hidden bg-[#132038]">
            ...
            <div className="relative z-10 text-center px-4 max-w-6xl mx-auto">
              <div className="mb-8">
                <div id="hero-badge" data-animate
                     className={`inline-block mb-6 transition-all duration-700 ${isVisible['hero-badge'] ? 'animate-expand-center' : 'opacity-0 scale-[0.3]'}`}>
      ...
      return (
        <div className="min-h-screen bg-gradient-to-b from-[#132038] to-[#0a1628] py-32 relative overflow-hidden">
          ...
          <div className="max-w-7xl mx-auto px-4 relative z-10">
            ...
            {[
              { key: 'students', label: 'STUDENTS',  sub: 'THE ENGINEERS', members: teamMembers.students },
              { key: 'mentors',  label: 'MENTORS',   sub: 'THE GUIDES',    members: teamMembers.mentors },
              { key: 'coaches',  label: 'COACHES',   sub: 'THE LEADERS',   members: teamMembers.coaches },
            ].filter(g => g.members.length > 0).map((group, gi) => (
              <div key={group.key} id={`${group.key}-section`} data-animate
                   className={`mb-24 transition-all duration-700 ${isVisible[`${group.key}-section`] ? 'animate-fade-in' : 'opacity-0'}`}
                   style={{ transitionDelay: `${300 + gi * 100}ms` }}>
                <div className="mb-12">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                  {group.members.map((member, i) => (
                    <div key={i} className="text-center"
                      ...
                      <TeamMemberCard member={member} size="large" showRookie={group.key === 'students'} />
                      <div className="mt-4">
                    ...
                    <div className="grid grid-cols-2 gap-6">
                      {[1, 2, 3, 4].map(i => (
                        <div key={i} className="aspect-square bg-gradient-to-br from-blue-800 to-orange-800 flex items-center justify-center text-white text-5xl font-bold overflow-hidden hover:scale-[1.05] transition-all duration-300 border-2 border-[#A2A9B1] hover:border-orange-600 group relative"
                             style={{ clipPath: 'polygon(0 0, calc(100% - 16px) 0, 100% 16px, 100% 100%, 0 100%)' }}>
                          <RobotImage src={`/data/robots/matchstick-${i}.jpg`} alt={`Matchstick detail ${i}`} fallbackText={i.toString()} />
                          <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 animate-pulse-glow"
                      ...
                      <div className="relative z-10">
                        ...
                        {item.content ? (
                          ...
                          <div className="flex gap-4">
                            {[
                              ['https://github.com/wolverine-robotics', Github],
                              ['https://www.linkedin.com/company/wolverine-robotics/', Linkedin],
                              ['https://www.instagram.com/wolverine_robotics/', Instagram],
                            ].map(([href, Icon], i) => (
              ...
              <div id="contact-form" data-animate
                ...
                <div className="relative bg-[#1a2847] p-8 border-2 border-orange-600 group hover:border-orange-500 transition-all duration-500 hover:scale-[1.02]"
                  ...
                  <form className="space-y-6 relative z-10">
                    {[['NAME', 'text'], ['EMAIL', 'email']].map(([label, type]) => (
                      <div key={label} className="animate-fade-in-up">
                        ...
                        <input type={type} className="w-full px-4 py-4 bg-[#0f1629] text-white border-2 border-[#A2A9B1] focus:border-orange-600 focus:outline-none transition-colors duration-300 focus:scale-[1.02]"
                               style={{ clipPath: 'polygon(0 0, calc(100% - 8px) 0, 100% 8px, 100% 100%, 0 100%)' }} />
                      </div>
                    ))}
                    <div className="animate-fade-in-up">
                      <label className="block text-white font-bold text-sm mb-2 tracking-wider">MESSAGE</label>
                      <textarea rows="6" className="w-full px-4 py-4 bg-[#0f1629] text-white border-2 border-[#A2A9B1] focus:border-orange-600 focus:outline-none transition-colors duration-300 resize-none focus:scale-[1.02]"
                                style={{ clipPath: 'polygon(0 0, calc(100% - 8px) 0, 100% 8px, 100% 100%, 0 100%)' }} />
                    </div>
                    <div className="animate-fade-in-up">
    ...
    if (displayPage === 'scouting') {
      return <ScoutingPageComponent />;
    }
